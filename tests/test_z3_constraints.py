"""Constraint coverage tests for Z3 backend.

These tests are skipped if Z3 is not available.
"""
...
Z3_AVAILABLE = get_backend("z3") is not None

pytestmark = pytest.mark.skipif(
    not Z3_AVAILABLE,
    reason="Z3 backend not available"
)
...
class TestNoOverlapConstraint:
    """Tests for NoOverlap constraint."""

    def test_nooverlap_constant_lengths(self):
        """Test NoOverlap with constant lengths."""
        n = 3
        starts = VarArray(size=n, dom=range(10))
        lengths = [2, 3, 2]

        satisfy(NoOverlap(origins=starts, lengths=lengths))
        ...
        for i in range(n):
            for j in range(i + 1, n):
                assert (
                    sol[i] + lengths[i] <= sol[j] or sol[j] + lengths[j] <= sol[i]
                ), f"Intervals {i} and {j} overlap"
